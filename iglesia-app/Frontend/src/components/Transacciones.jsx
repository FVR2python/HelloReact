  ...
  return (
    <div className="space-y-6 p-4">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-white p-6 rounded-2xl shadow border">
        ...
        <input type="number" name="monto_total" value={formData.monto_total} onChange={handleChange} placeholder="Monto Total *" className="input-form" required />

        <select name="estado" value={formData.estado} onChange={handleChange} className="input-form" required>
          <option value="pendiente">Pendiente</option>
          <option value="completo">Completo</option>
        </select>

        <input type="date" name="fecha_transaccion" value={formData.fecha_transaccion} onChange={handleChange} className="input-form" required />
            ...
            {transacciones.length > 0 ? (
              transacciones.map(t => (
                <tr key={t.id_transaccion} className="hover:bg-gray-100">
                  ...
                  <td className="table-td">S/ {parseFloat(t.monto_total).toFixed(2)}</td>
                  <td className="table-td">
                    <span className={`px-2 py-1 rounded-full text-xs font-bold ${t.estado === 'completo' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                      {t.estado}
                    </span>
                  </td>
