...
function ModalGestionInscripcion({ sacramento, onClose }) {
  ...
  const estadoInicial = {
    id_persona: '', fecha_matricula: '', descripcion: '',
    estado_matricula: 1, id_padrino: '', id_madrina: '',
    observaciones: '', id_sacramento: '', id_persona_rol_conyuge: ''
  };
  ...
  const obtenerConyuges = useCallback(async () => {
    try {
      const res = await fetch('http://localhost:5000/personas-rol/Cónyuge');
      const data = await res.json();
      setConyuges(data);
    } catch (error) {
  ...
  const obtenerPadrinos = useCallback(async () => {
    try {
      const res = await fetch('http://localhost:5000/personas-rol/Padrino');
      const data = await res.json();
      setPadrinos(data);
    } catch (error) {
  ...
  const obtenerMadrinas = useCallback(async () => {
    try {
      const res = await fetch('http://localhost:5000/personas-rol/Madrina');
      const data = await res.json();
      setMadrinas(data);
    } catch (error) {
  ...
  const sacramentosConConyuge = useMemo(() =>
    ['matrimonio']
  , []);
  ...
  useEffect(() => {
    ...
    sacramento, sacramentosConPadrinos, sacramentosConConyuge,
    obtenerPersonasPorSacramento, obtenerConyuges, obtenerPadrinos, obtenerMadrinas, obtenerInscripciones
  ]);
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-white p-4 rounded-xl border shadow mb-6">
        <div>
          ...
          <select name="id_persona" value={formData.id_persona} onChange={handleChange} className="input-form" required>
            ...
            {personas.map(p => <option key={p.id_persona} value={p.id_persona}>{p.nombre_completo} - {p.dni}</option>)}
          </select>
        </div>
        <div>
        ...
        {mostrarConyuge && (
          <div>
            ...
            <select name="id_persona_rol_conyuge" value={formData.id_persona_rol_conyuge} onChange={handleChange} className="input-form">
              <option value="">Seleccione...</option>
              {conyuges.map(c => <option key={c.id_persona_rol} value={c.id_persona_rol}>{c.nombre_persona}</option>)}
            </select>
          </div>
        )}
          ...
          <>
            <div>
              ...
              <select name="id_padrino" value={formData.id_padrino} onChange={handleChange} className="input-form">
                <option value="">Seleccione...</option>
                {padrinos.map(p => <option key={p.id_persona_rol} value={p.id_persona_rol}>{p.nombre_persona}</option>)}
              </select>
            </div>
            <div>
              ...
              <select name="id_madrina" value={formData.id_madrina} onChange={handleChange} className="input-form">
                <option value="">Seleccione...</option>
                {madrinas.map(m => <option key={m.id_persona_rol} value={m.id_persona_rol}>{m.nombre_persona}</option>)}
              </select>
            </div>
          </>
        )}
        ...
        <div className="md:col-span-2 flex justify-end gap-3 mt-2">
          <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">{editando ? 'Actualizar' : 'Registrar'}</button>
          {editando && <button type="button" onClick={() => { setEditando(null); resetForm(); }} className="px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition">Cancelar</button>}
        </div>
      </form>
        ...
        <table className="min-w-full text-sm text-center">
          ...
          <tbody>
            {inscripciones.length > 0 ? (
              ...
              <tr>
                <td colSpan={6 + (mostrarConyuge ? 1 : 0) + (mostrarPadrinos ? 2 : 0)} className="text-center py-4 text-gray-500">
