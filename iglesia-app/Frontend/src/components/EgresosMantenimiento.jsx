        ...
        >
          ...
          {transacciones.map(t => (
            <option key={t.id_transaccion} value={t.id_transaccion}>
              {t.descripcion} - S/ {parseFloat(t.monto_total).toFixed(2)} ({t.estado})
            </option>
          ))}
        </select>
            ...
            {egresos.length > 0 ? (
              egresos.map(e => (
                <tr key={e.id_egreso} className="hover:bg-gray-100">
                  <td className="py-2 px-2">{formatearFecha(e.fecha_egreso)}</td>
                  <td className="py-2 px-2">{e.proveedor}</td>
                  <td className="py-2 px-2">{e.nombre_invent}</td>
                  <td className="py-2 px-2">{e.descripcion_transaccion} - S/ {parseFloat(e.monto_total).toFixed(2)} ({e.estado})</td>
