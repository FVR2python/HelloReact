...
def listar_grupos():
    try:
        ...
        cursor.execute("""
            SELECT g.*, s.nombre AS nombre_sacramento, p.nombre_prrq AS nombre_parroquia
...
def crear_grupo():
    ...
    try:
        ...
        cursor.execute("""
            ...
            VALUES (%s, %s, %s, %s, %s)
        """, (data['nombre_grupo'], data['fecha_inicio'], data['fecha_fin'], data['id_sacramento'], data['id_parroquia']))
        conn.commit()
        cursor.close()
        conn.close()
...
def actualizar_grupo(id_grupo):
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE id_grupo=%s
        """, (data['nombre_grupo'], data['fecha_inicio'], data['fecha_fin'], data['id_sacramento'], data['id_parroquia'], id_grupo))
        conn.commit()
        cursor.close()
        conn.close()
