...
function EventosLiturgicos() {
  ...
  const handleSubmit = async e => {
    e.preventDefault();
    const url = editando ? `/eventos_liturgicos/${editando}` : '/eventos_liturgicos';
  ...
  const handleEliminar = async id => {
    ...
    if (result.isConfirmed) {
      const res = await fetch(`/eventos_liturgicos/${id}`, { method: 'DELETE' });
      if (res.ok) {
        obtenerEventos();
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          ...
          >
            ...
            <option value="misa_dominical">Misa Dominical</option>
