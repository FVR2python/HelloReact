  ...
  return (
    <div className="space-y-6 p-4">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 bg-white p-6 rounded-2xl shadow border">
        <select name="accion" value={formData.accion} onChange={handleChange} className="input-form" required>
          <option value="">Seleccione acción</option>
          <option value="editar">Editar</option>
          <option value="anular">Anular</option>
        </select>
        <input type="date" name="fecha_accion" value={formData.fecha_accion} onChange={handleChange} className="input-form" required />
        <input type="text" name="observacion" value={formData.observacion} onChange={handleChange} className="input-form" placeholder="Observación" />
        <select name="id_transaccion" value={formData.id_transaccion} onChange={handleChange} className="input-form" required>
          ...
          {transacciones.map(t => (
            <option key={t.id_transaccion} value={t.id_transaccion}>{t.descripcion} - S/ {parseFloat(t.monto).toFixed(2)}</option>
          ))}
        </select>
        <input type="text" name="usuario" value={formData.usuario} onChange={handleChange} className="input-form" placeholder="Usuario" />
        <input type="text" name="ip" value={formData.ip} onChange={handleChange} className="input-form" placeholder="IP" />
        <input type="text" name="modulo" value={formData.modulo} onChange={handleChange} className="input-form" placeholder="Módulo" />
        <input type="text" name="datos_anteriores" value={formData.datos_anteriores} onChange={handleChange} className="input-form" placeholder="Datos anteriores" />
        <input type="text" name="datos_nuevos" value={formData.datos_nuevos} onChange={handleChange} className="input-form" placeholder="Datos nuevos" />
        <div className="col-span-full flex justify-end gap-3 mt-2">
          {editando && (
            <button type="button" onClick={handleCancelar} className="px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition">Cancelar</button>
          )}
          <button type="submit" className={`px-4 py-2 text-white rounded-lg transition ${editando ? 'bg-yellow-500 hover:bg-yellow-600' : 'bg-blue-600 hover:bg-blue-700'}`}>
        ...
        <table className="min-w-full text-sm text-center">
          <thead className="bg-blue-600 text-white uppercase text-xs">
            <tr>
              ...
              <th className="py-3 px-2">Datos Ant.</th>
              <th className="py-3 px-2">Datos Nuevos</th>
            ...
            {auditorias.length > 0 ? (
              auditorias.map(a => (
                <tr key={a.id_auditoria} className="hover:bg-gray-100">
                  ...
                  <td className="py-2 px-2">{a.usuario}</td>
                  <td className="py-2 px-2">{a.ip}</td>
                  <td className="py-2 px-2">{a.modulo}</td>
                  <td className="py-2 px-2">{a.datos_anteriores}</td>
                  <td className="py-2 px-2">{a.datos_nuevos}</td>
